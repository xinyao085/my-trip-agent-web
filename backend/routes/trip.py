...
from ..memory.redis import save_conversation, save_message, get_conversations, get_messages, delete_conversation
from ..memory.rag import retrieve
from ..agents.react import fetch_fresh_data, extract_new_params, stream_planner

router = APIRouter()

# 进程内字典存储活跃会话运行时状态（重启后清空，持久化记录在 Redis）
...
async def restore_session(session_id: str) -> dict | None:
    """从 Redis 重建会话运行时状态，供关闭后重新继续修改计划使用。"""
    from ..memory.redis import _get_redis
    r = _get_redis()
    data = await r.hgetall(f"conv:{session_id}")
    if not data:
        return None
    ...
    # 用 Redis 中存储的对话记录重建 LangChain 消息历史
    stored = await get_messages(session_id)
    planner_history: list = [SystemMessage(content=_PLANNER_SYSTEM_PROMPT)]
    for msg in stored:
        if msg["role"] == "user":
            planner_history.append(HumanMessage(content=msg["content"]))
        elif msg["role"] == "ai":
            planner_history.append(AIMessage(content=msg["content"]))
...
async def plan_trip(req: PlanRequest):
    async def generate() -> AsyncGenerator[str, None]:
        try:
            ...
            knowledge = retrieve(f"{req.city} {req.days}天旅行 {req.preferences}")
            knowledge_line = f"旅行知识库参考（排队、季节、文化等实用建议）：\n{knowledge}\n\n" if knowledge else ""
            user_query = (
            ...
            # 持久化到长期记忆
            user_summary = f"📍 {req.city}  ·  {req.days} 天{'  ·  ' + req.preferences if req.preferences else ''}"
            await save_conversation(session_id, req.city, req.days, req.preferences)
            await save_message(session_id, "user", user_summary)
            await save_message(session_id, "ai", full_ai_text)
...
async def chat(req: ChatRequest):
    ...
    if not session:
        session = await restore_session(req.session_id)
        if not session:
    ...
    async def generate() -> AsyncGenerator[str, None]:
        try:
            ...
            await save_message(req.session_id, "ai", full_ai_text)
...
@router.get("/history", summary="获取历史对话列表")
async def list_history():
    return await get_conversations()


@router.get("/history/{session_id}", summary="获取指定历史对话的消息")
async def get_history(session_id: str):
    msgs = await get_messages(session_id)
    if not msgs:
        raise HTTPException(status_code=404, detail="未找到该历史对话")
    return msgs


@router.delete("/history/{session_id}", summary="删除指定历史对话")
